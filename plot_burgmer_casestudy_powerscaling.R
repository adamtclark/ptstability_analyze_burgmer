...
dlst<-c("Chlamydomonas.terricola_LSA_long_powerscale.rda", "Chlamydomonas.terricola_LVA_long_powerscale.rda")
...
pdf("plotout/burgmer_examples_powerscaling_220801.pdf", width=6, height=5, colormodel = "cmyk", useDingbats = FALSE)
nm= 2
...
for(ii in 1:length(dlst)) {
  ...
  plot(c((dat$time[yps][libuse_y[1,1]:libuse_y[1,2]])[Euse], max(dat$time[yps])), c(0, ymax[ii]), xlab="", ylab="", type="n", xaxs="i")
  ...
  if(ii==2) {
    ...
    lfuse<-exp
    plot(density(lfuse(smp_EDM[,1]), bw = diff(range(lfuse(smp_EDM[,1])))/5,
    ...
    tmpd = density(lfuse(tmp1), bw = diff(range(lfuse(tmp1)))/5,
    ...
    cbxfun()
    box()
    title("c.", line = axln, cex.main=axcx, adj=axadj)
  }
  ...
  if(ii==2) {
    ...
    xsq1 = seq(min(tmp11c,na.rm=T), max(tmp11c, na.rm=T), length=100)[-1]
    xsq2 = seq(min(tmp11c_2,na.rm=T), max(tmp11c_2, na.rm=T), length=100)[-1]
    var1_power = var2_power = matrix(nrow=length(xsq1), ncol = ncol(tmp11c_2))
    
    for(iii in 1:length(tmp11)) {
      var1_power[,iii] = exp(tmp11[iii])*(xsq1)^exp(tmp11b[iii])
      var2_power[,iii] = exp(tmp11_2[iii])*(xsq2)^exp(tmp11b_2[iii])
    }
    
    varplot1 = t(apply(var1_power, 1, function(x)
      {c(mean(x, na.rm=T)+sd(x,na.rm=T)*c(-1,0,1))}))
    varplot2 = t(apply(var2_power, 1, function(x)
      {c(mean(x, na.rm=T)+sd(x,na.rm=T)*c(-1,0,1))}))
    
    plot(range(xsq1, xsq2, na.rm=T), log = "xy",
         range(c(varplot1, varplot2, na.rm=T)),
         main="", xlab="", ylab="", lwd=1.5, type = "n")
    
    lines(xsq1, varplot1[,2], lwd=2, col = mcol2)
    lines(xsq2, varplot2[,2], lwd=2, col = collst[dcolps])
    
    polygon(c(xsq1, rev(xsq1)), c(varplot1[,1], rev(varplot1[,3])),
            col=adjustcolor(mcol2, alpha.f = 0.5), border = NA)
    polygon(c(xsq2, rev(xsq2)), c(varplot2[,1], rev(varplot2[,3])),
            col=adjustcolor(collst[dcolps], alpha.f = 0.5), border = NA)
    
    abline(a=0, b=1, lty=3)
    mtext(expression(paste("Abundance")), side = 1, line = 2.6)
    mtext(expression(paste("Proc. Noise Var, ", sigma[italic(P)]^2)), side = 2, line = 2.6)
