...
dlst<-dir("datout/")
dlst<-sort(unique(dlst[grep(".rda", dlst)]))
...
for(ii in 1:length(dlst)) {
  ...
  cbxfun<-function(dw1=0.05, dw2=0.18) {
    xp<-par("usr")[1:2]
    yp<-par("usr")[3:4]
    dxs<-(xp[2]-xp[1])*dw1
    dyx<-(yp[2]-yp[1])*dw2
  ...
  pdf(paste("plotout/", simname, ".pdf", sep=""), width=10, height=6, colormodel = "cmyk", useDingbats = FALSE)
    ...
    mtext("Estimated Std. Abundance", side = 2, line = 2.6)
    cbxfun()
    box()
    title("a.", line = axln, cex.main=axcx, adj=axadj)
    ...
    for(i in 1:(nrow(libuse_y)+1)) {
      ...
      if(i<=nrow(libuse_y)) {
        ...
        polygon(c(tm, rev(tm)), c(tmdat_ci[,i,1], rev(tmdat_ci[,i,3])), col=adjustcolor(collst[i], alpha.f = 0.8))
      }
    }
    abline(h=c(0,1), lty=3)
    mtext("Time (days)", side = 1, line = 2.6)
    mtext("Estimated Pr[Extinction]", side = 2, line = 2.6)
    cbxfun()
    box()
    title("b.", line = axln, cex.main=axcx, adj=axadj)
    
    
    #parameters
    #axslst<-c(0.01, 0.02, 0.05, 0.2, 0.1, 0.5, 1, 2, 5)
    lfuse<-exp
    
    plot(density(lfuse(smp_EDM[,1]), bw = diff(range(lfuse(smp_EDM[,1])))/20,
                 from=lfuse(minvUSE_edm[1]), to=lfuse(maxvUSE_edm[1])),
         main="", xlab="", ylab="", lwd=1.5, axes=F)
    axis(2)
    axis(1)
    #axis(1, at=log(axslst), axslst)
    box()
    abline(v=mean(lfuse(smp_EDM[,1])), lwd=2, col="blue")
    abline(h=0, lty=3)
    ...
    #axis(1, at=log(axslst), axslst)
    box()
    abline(v=mean(lfuse(smp_EDM[,2])), lwd=2, col="blue")
    abline(h=0, lty=3)
    ...
    #axis(1, at=log(axslst), axslst)
    box()
    abline(v=mean(lfuse(smp_EDM[,3])), lwd=2, col="blue")
    abline(h=0, lty=3)
    ...
    mtext("Raw", side = 2, line = 2.6)
    mtext("Estimated", side = 1, line = 2.6)
    cbxfun()
    box()
    title("g.", line = axln, cex.main=axcx, adj=axadj)
    
    if(FALSE) {
      #analytical mortality prediction
      ...
      mormu<-mean(pmedm_tot[,ncol(pmedm_tot)],na.rm=T)
      abline(v=mormu, lwd=2, lty=2)
      
      mtext("Extrapolated Pr[Extinction]", side = 1, line = 2.6)
      mtext("Density", side = 2, line = 2.6)
    } else {
      #time to extinction
      ...
      for(i in 1:ncol(text_extrap)) {
        dns[[i]]<-density(text_extrap[,i], from = dnsrng[1], na.rm = T)
      }
...
sumdat<-data.frame(sp=trtsplst[commArg_ps,1], trt=trtuse,
                   ...
                   mormu, morsd=sd(pmedm_tot[,ncol(pmedm_tot)],na.rm=T))
