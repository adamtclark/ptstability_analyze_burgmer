...
for(iclu in c(13,15)) {
  #LSA and LVA treatments:
  #low temperature, static or variable temperature trend, predators absent
  simname<-paste(c(trtsplst[iclu,]), collapse = "_")
  print(simname)
  # load data
  ...
  # set priors
  minvUSE_edm<-c(log(0.001), log(0.001), log(0.01))
  maxvUSE_edm<-c(log(2), log(2), log(3))
  #Parameters are (for abundance x): 
  #1. Slope term for observation error, such that sd(obs) = exp(o[1])*x
  #2-3. Taylor Power Law paramters for process noise, such that
        #sd(proc) = sqrt(exp(p[1])*x^exp(p[2]))
  ...
  #piece-wise function needed for handling spatial replicates
  ...
  #check fit
  summary(out_EDM, start = floor(niter/5))
  ...
  save(list = c("out_EDM", "smp_EDM", "simout", "sdout", "simout_smp", "simout_smp_noproc", "pmedm_analy"),
       file = paste("datout/", simname, "_long_powerscale.rda", sep=""), version = 2)
}
